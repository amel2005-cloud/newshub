    ...
    <PublicLayout>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <article className="lg:col-span-2 bg-white rounded-xl p-6 shadow-sm">
          ...
          <h1 className="text-navy font-extrabold text-2xl md:text-3xl leading-tight mb-4">
            {news.title}
          </h1>
          ...
          <div className="flex items-center gap-4 text-gray-500 text-sm mb-5 flex-wrap">
            ...
            <span>
              {news.published_at
                ? format(new Date(news.published_at), "d MMMM yyyy", {
                    locale: id,
                  })
                : ""}
            </span>
        ...
        <aside className="bg-white rounded-xl p-4 shadow-sm h-fit">
          <h3 className="font-bold text-navy text-lg border-l-4 border-red pl-3 mb-4">
            Berita Terkait
          </h3>
          <div className="space-y-4">
            {related.length === 0 && (
              <p className="text-gray-400 text-sm">Tidak ada berita terkait.</p>
            )}
            {related.map((n) => (
              <NewsCard key={n.id} news={n} size="small" />
            ))}
          </div>
        </aside>
      </div>
